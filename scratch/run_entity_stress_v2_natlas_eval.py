...
def parse_args():
    parser = argparse.ArgumentParser(description="Run direct N-ATLaS ASR on YPIT entity stress v2.")
    parser.add_argument("--metadata", default="entity_stress_v2_metadata.json", type=Path)
    parser.add_argument("--audio-dir", default="entity_stress_v2_audio_fixed", type=Path)
    parser.add_argument("--out", default="entity_stress_v2_natlas_full_results.json", type=Path)
...
def main():
    ...
    for index, row in enumerate(metadata, start=1):
        ...
        print(f"\n[{index:03d}/{len(metadata)}] {row['id']} {row['audio_filename']}")
        audio, _ = librosa.load(audio_path, sr=16000, mono=True)
        inputs = processor(audio, sampling_rate=16000, return_tensors="pt", return_attention_mask=True)
        input_features = inputs.input_features.to(device, dtype=dtype)
        start = time.perf_counter()
        generated_ids = model.generate(input_features, forced_decoder_ids=forced_ids, max_new_tokens=128)
